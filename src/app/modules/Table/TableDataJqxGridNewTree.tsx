...
// import JqxTreeeGrid, { IGridProps, jqx } from 'jqwidgets-scripts/jqwidgets-react-tsx/jqxtreegrid';
...
interface ITableDataJqxGridNew {
  ...
  onClickAddlaporan?: any; // event ketika add data ex: mumunculkan modal atau reedirect ke form page
  ...
  onClickDelete?: any; // event ketika Update ex: mumunculkan modal atau reedirect ke form page

}

const TableDataJqxGridNewTree: React.FC<ITableDataJqxGridNew> = React.memo(({
  ...
  const [source] = useState<any>(() => {
    const sourceObj: any = {
      ...
      id: primaryKey,
      hierarchy: {
        keyDataField: { name: primaryKey },
        parentDataField: { name: "id_parent_lokasi" }
    },
      datatype: 'json',
      root: 'results',
      cache: false,
    ...
    if (!toolbarRendered.current) {
      ...
      const buttonsContainer = (
        <div style={{ overflowX: 'auto', overflowY: 'hidden', whiteSpace: 'nowrap', position: 'relative', margin: '5px' }}>
          ...
          {addlaporan && (
            <JqxButton
              onClick={handleAddClicklaporan}
              theme={themeJqx}
              width={120}
              height={20}
              style={{ ...buttonStyle, backgroundColor: '#d9db27', color: '#fff' }}
              className="jqx-button-custom" // Kelas CSS untuk efek hover
            >
  ...
  /** HANDLE TAMBAH DATA */
  const handleAddClicklaporan = (e: any) => {
...
export default TableDataJqxGridNewTree;