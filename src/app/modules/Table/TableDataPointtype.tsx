...
export default function TableData({
  ...
  const handleRowsChecked = (checked: any) => {
    if (rowSelectType === 'radio') {
      // Untuk radio button, hanya simpan satu item yang terpilih
      const selected = checked[0]; // Ambil item yang terpilih

      setCheckedRows([selected]); // Perbarui checkedRows dengan satu item
      setCheckedRowsKey({ [selected?.key]: true }); // Perbarui checkedRowsKey dengan key dari item yang terpilih
      if (onCheckedRows) onCheckedRows([selected]); // Panggil onCheckedRows dengan item yang terpilih
    ...
    if (checkSize > 0 && checkSize < checkRows) {
      // Kasus di mana item yang dipilih kurang dari item yang ditandai
      ...
      const listsUpdate = checkedRows
        ?.filter((item: any) => !removed.includes(item?.id))
        ?.map((item: any) => {
          return { ...item };
        });
      ...
      // Kasus di mana item yang dipilih sama dengan atau lebih dari item yang ditandai
