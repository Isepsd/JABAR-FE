...
interface ITableDataJqxGridEam {
  sortBy?: any;
...
const TableDataJqxGridEam: React.FC<ITableDataJqxGridEam> = React.memo(
  ({
    ...
    const handleRowsSelected = (event: any) => {
      if (selectionmode === "checkbox") {
        ...
        // //console.log("handleRowsSelected");
      ...
      if (!toolbarRendered.current) {
        ...
        const buttonsContainer = (
          ...
          >
            ...
            {addbtnMutasiTemp && (
              <JqxButton
                onClick={handleAddClickInsertTemp}
                theme={themeJqx}
                width={70}
                height={15}
                style={{
    ...
    const handleAddClickInsertTemp = (e: any) => {
      if (onClickAdd) {
        onClickAdd(e);
      } else {
        const target = typeof addbtn == "boolean" ? "add" : addbtn;
    ...
    /** HANDLE MUTAS DATA */
    const handleaddMutasiClick = () => {
      console.log(primaryKey["id_ref_aset"]);
      if (primaryKey["id_ref_aset"]) {
        // if (onClickUpdate) {
        //   onClickUpdate(e);
        //   searchParams.delete(ids);
        //   searchParams.append(ids, get(rowsSelected.current, primaryKey));
        //   setSearchParams(searchParams);
        // } else {
        //   navigate(`mutasi/${rowsSelected.current[primaryKey]}`);
        // }
        navigate(`mutasi/${primaryKey["id_ref_aset"]}`);
      } else {
        ShowAlert();
      }
    };
    ...
    //console.log("component TableDataJqxGridEam has rendered");
      ...
      <>
        ...
        <div className="p-1">
          ...
          >
            <div>
              {" "}
              <i className="fa-regular fa-circle-question me-2"></i> Konfirmasi
              delete{" "}
            </div>
            <div>
              <div>
...
export default TableDataJqxGridEam;
