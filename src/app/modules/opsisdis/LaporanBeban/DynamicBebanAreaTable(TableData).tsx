...
function DynamicBebanAreaTable({
  ...
  const handleRespDataApi = (data: any) => {
    ...
    dataTableValue = data.map((item: any, index: number) => {
      ...
      item.id = index
      // item.nama_uid = nama_uid
      item.nama_uid = item.nama_uid || "Default UID";
      // item.nama_uid = "Your Default Text";
  ...
  // const ensureColumnsText = (columns:any) => {
  //   return columns.map(column => ({
  //     ...column,
  //     // text: column.text ? column.text : 'Default Text'
  //     text: column.Header || 'Header'
  //   }));
  // };
  ...
  return (
    <>
      ...
      {/* <TableDataJQWidget
        columnsConfig={ensureColumnsText(columns)}
        respDataApi={handleRespDataApi}
        rowData={dataRows}
        path={path}
        primaryKey={primaryKey}
