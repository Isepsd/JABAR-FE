...
export default function WoDraftingForm({ handleClose }: any) {
  ...
  const handleUploadedSebelum = (e: any) => {
    setFolderBefore(e.data);
    setValue('foto_sebelum', e.data);
  }
  const handleUploadedSesudah = (e: any) => {
    setFolderAfter(e.data)
    setValue('foto_sesudah', e.data);
  }

  const onSubmitForm = (data: IFasopNoWO) => {
    data.foto_sebelum = folderBefore ? folderBefore : watch('foto_sebelum')
    data.foto_sesudah = folderAfter ? folderAfter : watch('foto_sesudah')
        ...
        <Form noValidate onSubmit={handleSubmit(onSubmitForm)}>
          <Row className='mb-3'>
            ...
            <Col md="12">
              ...
              <Form.Group className='mt-3 d-flex align-items-center'>
                <Form.Label className='mb-4' style={{ minWidth: '120px' }}>
                  Dokumen Sebelum
                </Form.Label>
                <div className='d-flex align-items-center flex-grow-1'>
                  ...
                  <div style={{ width: '97%' }}>
                    <InputUploadNew
                      ...
                      path='upload_image'
                      rootFolder='portal'
                      previewUrl={folderBefore ? folderBefore : watch('foto_sebelum')}
                      folder="drafting_wo"
                      accept="image/*"
                      // onChange={(file) => setFolderBefore(file)}
                      onUploaded={handleUploadedSebelum}
                      setValue={setValue} // Ensure setValue is correctly passed
                    />
                  </div>
                </div>
                <Form.Control.Feedback type='invalid'>
                  {errors?.foto_sebelum?.message}
                </Form.Control.Feedback>
              </Form.Group>
                ...
                <div className='d-flex align-items-center flex-grow-1'>
                  ...
                  <div style={{ width: '97%' }}>
                    <InputUploadNew
                      ...
                      path='upload_image'
                      rootFolder='portal'
                      previewUrl={folderAfter ? folderAfter : watch('foto_sesudah')}
                      folder="drafting_wo"
                      accept="image/*"
                      // onChange={(file) => setFolderAfter(file)}
                      onUploaded={handleUploadedSesudah}
                      setValue={setValue} // Ensure setValue is correctly passed
                    />
                  </div>
                </div>
                <Form.Control.Feedback type='invalid'>
                  {errors?.foto_sesudah?.message}
                </Form.Control.Feedback>
              </Form.Group>
