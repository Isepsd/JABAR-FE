...
import { TRAFO_GDISTRIBUSI } from '@app/configs/react-table/master-jaringan.columns.config';
...
export default function JarTrafoGDPage() {
  ...
  const [columns, setColumns] = useState<any>(TRAFO_GDISTRIBUSI());
  ...
  return (
    <>
      ...
      <TableData
        ...
        // exportConfig={{path: API_PATH().master.management_upload.trafo_gd+'/download-excel', customParams:{}}}
        primaryKey={'id_ref_lokasi'}
        action={action}
        selected={dataSelected}
        deleteConfirmation
        filterParams={{ id_ref_jenis_lokasi: JENIS_LOKASI().trafo_gd, sort_by: '-tgl_update,id_ref_lokasi' }}
      ></TableData>
    </>
  );
}
