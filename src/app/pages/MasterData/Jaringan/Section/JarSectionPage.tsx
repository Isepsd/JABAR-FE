...
import { SECTION } from '@app/configs/react-table/master-jaringan.columns.config';
...
export default function JarSectionPage() {
  ...
  const [columns, setColumns] = useState<any>(SECTION());
  ...
  return (
    <>
      ...
      <TableData
        ...
        // exportConfig={{path: API_PATH().master.management_upload.section+'/download-excel', customParams:{}}}
        primaryKey={'id_ref_lokasi'}
        action={action}
        selected={dataSelected}
        deleteConfirmation
        // filterParams={{id_ref_jenis_lokasi: JENIS_LOKASI().section, sort_by:'-tgl_update,id_ref_lokasi'}}
        filterParams={{ id_ref_jenis_lokasi: JENIS_LOKASI().keypoint, fungsi_lokasi:'SECTION' }}
      ></TableData>
    </>
  );
}
