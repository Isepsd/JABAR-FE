...
import {
  IJaringanSection,
  JaringanSectionField,
} from '@app/interface/jaringan-section.interface';
...
export default function JarSectionForm() {
  ...
  const validationSchema = Yup.object().shape({
    ...
    // id_zone: Yup.number()
    //   .typeError('Belum pilih zone')
    //   .required('Belum pilih zone'),
    status_listrik: Yup.string()
      .nullable()
      .transform((_, v) => (v == '1' ? '1' : '0')),
    // lat: Yup.number()
    //   .typeError('Latitude harus number')
    //   .required('Latitude Wajib diisi'),
    // lon: Yup.number()
    //   .typeError('Longitude harus number')
  ...
  const onSubmitForm = (data: IJaringanSection) => {
    data.id_parent_lokasi = data?.id_zone;
    // data.id_ref_jenis_lokasi = JENIS_LOKASI().section;
    data.id_ref_jenis_lokasi = JENIS_LOKASI().keypoint;
    data.fungsi_lokasi = 'SECTION';
    // data.jenis_jaringan = '';
          ...
          <Row className='mb-3'>
            ...
            <Col md='7'>
              ...
              <Form.Group className='mt-3' controlId='phase'>
                ...
                <Form.Control {...register('phase')} isInvalid={errors.phase} />
                <Form.Control.Feedback type='invalid'>
