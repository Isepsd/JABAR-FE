...
export default function APIWAGatewayPage() {
  ...
  const handleReqs = async (item: any) => {
    ...
    let routes = "v1.2/instances/whatsapp/get-groups";
    setLoading(true);
    const trimmedUrl = url.substring(
      0,
      url.indexOf("v2/messaging/whatsapp/instances/message")
    );
  ...
  const getAllData = async () => {
    ...
    try {
      const req: any = await getAllByPath(
        API_PATH().master.fasop.whatsapp.bot,
        params,
        source.token
      );
      ...
      <Row>
        <Col md={12} className="mb-4">
          <Card className="card-widget">
            <Card.Header>
              <h5>List BOT WA Gateway for Get Groups Kontak</h5>
            </Card.Header>
            <Card.Body>
