...
import { MASTER_CUACA } from '@app/configs/react-table/master-opsisdis.columns.config';
...
export default function CuacaPage() {
  ...
  const path = API_PATH().master.opsisdis.rekap_padam.cuaca
  const [dataRows, setDataRows] = useState<any>([]);
  const [dataSelectedtion, setDataSelected] = useState<any>();
  const [action, setAction] = useState<any>();

  const [columns, setColumns] = useState<any>(MASTER_CUACA());
