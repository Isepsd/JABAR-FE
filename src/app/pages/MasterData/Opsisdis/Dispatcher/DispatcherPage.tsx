...
import { MASTER_DISPATCHER } from '@app/configs/react-table/master-opsisdis.columns.config';
...
export default function DispatcherPage() {
  ...
  const [path] = useState(API_PATH().master.opsisdis.rekap_padam.ref_ep_petugas)
  const [dataRows, setDataRows] = useState<any>([]);
  const [dataSelectedtion, setDataSelected] = useState<any>();
  const [action, setAction] = useState<any>();

  const [columns, setColumns] = useState<any>(MASTER_DISPATCHER());
  ...
  const handleEdit = (item: any) => {
    navigate(`edit/${item?.id_ref_ep_petugas}`);
  };
