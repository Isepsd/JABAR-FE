...
import { MASTER_PENYEBAB_GANGGUAN } from '@app/configs/react-table/master-opsisdis.columns.config';
import BadgeStatus from '@app/components/Status/BadgeStatus';
import TableData from '@app/modules/Table/TableData';
import { ROLE_ACCESS, ROLE_ACTION } from '@app/helper/auth.helper';

export default function OpsPenyebabGangguanPage() {
  const navigate = useNavigate();
  const path = API_PATH().master.opsisdis.rekap_padam.penyebab_gangguan
  const [dataRows, setDataRows] = useState<any>([]);
  const [dataSelectedtion, setDataSelected] = useState<any>();
  const [action, setAction] = useState<any>();
  const [roleActions, setRoleActions] = useState<any>({});
  const [columns, setColumns] = useState<any>(MASTER_PENYEBAB_GANGGUAN());
  ...
  const handleEdit = (item: any) => {
    navigate(`edit/${item?.id_ref_ep_penyebab_ggn}`);
  };
  ...
  const handleRespDataApi = (data: any, index: number) => {
    ...
    data?.forEach((item: any) => {
      dataTableValue.push({
        ...
        number: item.number,
        id: item?.id,
        kategori: item?.ref_ep_kat_ggn?.nama,
