...
import { PENGUKURAN_BEBAN_GH_COLUMN_JQWIDGET } from '@app/configs/jqwidget/pengukuran-beban-trafo.column.config';
import moment from 'moment';

export default function PbTrafoGIPage() {
  // const queryParams = qs.parse(location.search);
  const url = API_PATH().opsisdis.pengukuran_beban.gh;
  const urlCountNull = API_PATH().opsisdis.pengukuran_beban.gh;
  ...
  useEffect(() => {
    let fields: any = PENGUKURAN_BEBAN_GH_COLUMN_JQWIDGET(roleActions)
    setColumns(fields)
  }, [roleActions])
      ...
      {optionsGarduInduk && (
        <>
          <TableDataListAction
            ...
            column={false}
            add={false}
            module='Telemetring Gardu Hubung'
            filterLayout='card'
          ...
          <div>
            <TablePengukuranBeban
              ...
              pathService={url}
              primaryKey='id_trans_tm_gh'
              callBackCount={callBackCount}
              label='Telemetring Gardu Hubung'
              module='Telemetring Gardu Hubung'
              customFilter={customFilter}
              customParams={{ fungsi_lokasi: 'GH' }}
              ChangeColorRow={true}
