...
import {
  DAFTAR_PADAM__PENYULANG_COLUMN
  // LOAD_SUBSISTEM_FAKTOR_BULANAN_COLUMN 
} from '@app/configs/react-table/opsisdis/padam-penyulang/padam-penyulang';
import { DAFTAR_PADAM__PENYULANG_COLUMN_JQWidget } from '@app/configs/react-table/opsisdis/padam-penyulang/laporan-penyulang.column';
...
const tabOptions = [
  { label: 'Daftar Penyulang Padam', value: 'beban_perjam', format: 'DD/MM/YYYY HH:mm', pathService: API_PATH().opsisdis.gangguan_penyulang.daftar_penyulang_padam, column: DAFTAR_PADAM__PENYULANG_COLUMN(), primaryKey: 'id' },
...
export default function DaftarPenyulangPadam() {
            ...
            {(Object.keys(queryParams).length > 0) && (
              <Card.Body>
                {tabActive == "beban_perjam" &&
                  <DynamicBebanAreaTable pathService={API_PATH().opsisdis.gangguan_penyulang.daftar_penyulang_padam}
                    columnsConfig={DAFTAR_PADAM__PENYULANG_COLUMN_JQWidget()} primaryKey={tabActiveConf.primaryKey} tabActive={'beban_perjam'} label="beban_uid" />
                }
