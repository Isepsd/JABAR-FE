...
/** CONFIG */
import {
  TOTAL_GANGGUAN_PER_GI_COLUMN
} from '@app/configs/react-table/opsisdis/padam-penyulang/padam-penyulang';
import { TOTAL_GANGGUAN_PER_GI_COLUMN_JQWidget } from '@app/configs/react-table/opsisdis/padam-penyulang/laporan-penyulang.column';
...
const tabOptions = [
  // { label: 'Total Gangguan Per GI', value: 'beban_perjam', format: 'DD/MM/YYYY HH:mm', pathService: API_PATH().opsisdis.gangguan_penyulang.total_gangguan_per_gi, column: TOTAL_GANGGUAN_PER_GI_COLUMN(), primaryKey: 'id' },
  { label: 'Total Gangguan Per GI', value: 'beban_perjam', format: 'DD/MM/YYYY HH:mm', pathService: API_PATH().opsisdis.padam_penyulang.padam_5_menit_area, column: TOTAL_GANGGUAN_PER_GI_COLUMN(), primaryKey: 'id' },
]
export default function TotalGangguanPerGI() {
            ...
            {(Object.keys(queryParams).length > 0) && (
              ...
              <Card.Body>
                {tabActive == "beban_perjam" &&
                  // <DynamicBebanAreaTable pathService={API_PATH().opsisdis.gangguan_penyulang.total_gangguan_per_gi}
                  <DynamicBebanAreaTable pathService={API_PATH().opsisdis.padam_penyulang.padam_5_menit_area}
                    columnsConfig={TOTAL_GANGGUAN_PER_GI_COLUMN_JQWidget()} primaryKey={tabActiveConf.primaryKey} tabActive={'beban_perjam'} label="beban_uid" />
                }
              </Card.Body>
            )}
