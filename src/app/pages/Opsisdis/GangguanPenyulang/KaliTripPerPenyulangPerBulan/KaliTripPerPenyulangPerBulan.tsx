...
const tabOptions = [
  { label: 'Kali Trip Per Penyulang Per Bulan', value: 'beban_bulanan', format: 'MM/YYYY', pathService: API_PATH().opsisdis.gangguan_penyulang.kali_trip_per_penyulang_per_bulan, column: KALI_TRIP_PER_PENYULANG_PER_BULAN_COLUMN(), primaryKey: 'id' },
]
export default function KaliTripPerPenyulangPerBulan() {
            ...
            {(Object.keys(queryParams).length > 0) && (
              <Card.Body>
                {tabActive == "beban_bulanan" &&
                  <DynamicBebanAreaTable pathService={API_PATH().opsisdis.gangguan_penyulang.kali_trip_per_penyulang_per_bulan}
                    columnsConfig={KALI_TRIP_PER_PENYULANG_PER_BULAN_COLUMN_JQWidget()} primaryKey={tabActiveConf.primaryKey} tabActive={'beban_bulanan'} label="beban_uid" />
                }
              </Card.Body>
            )}
