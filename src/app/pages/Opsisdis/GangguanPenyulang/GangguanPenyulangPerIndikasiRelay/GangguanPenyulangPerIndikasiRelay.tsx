...
/** CONFIG */
import {
  GANGGUAN_PENYULANG_PER_INDIKASI_RELAY_COLUMN
} from '@app/configs/react-table/opsisdis/padam-penyulang/padam-penyulang';
import { GANGGUAN_PENYULANG_PER_INDIKASI_RELAY_COLUMN_JQWidget } from '@app/configs/react-table/opsisdis/padam-penyulang/laporan-penyulang.column';
...
const tabOptions = [
  { label: 'Gangguan Penyulang Per Indikasi Relay', value: 'beban_perjam', format: 'DD/MM/YYYY HH:mm', pathService: API_PATH().opsisdis.laporan_beban.uid.jam, column: GANGGUAN_PENYULANG_PER_INDIKASI_RELAY_COLUMN(), primaryKey: 'id' },
]
export default function GangguanPenyulangPerIndikasiRelay() {
              ...
              <Card.Body>
                {tabActive == "beban_perjam" &&
                  <DynamicBebanAreaTable pathService={API_PATH().opsisdis.laporan_beban.uid.jam}
                    columnsConfig={GANGGUAN_PENYULANG_PER_INDIKASI_RELAY_COLUMN_JQWidget()} primaryKey={tabActiveConf.primaryKey} tabActive={'beban_perjam'} label="beban_uid" />
                }
              </Card.Body>
            )}
