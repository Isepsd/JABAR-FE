...
const mappingBarChart = (data: any) => {
  ...
  data.forEach((item: any) => {
    // res.menit_padam.push(convertToMinutes(item.menit_padam));
    res.menit_padam.push({
      y: convertToMinutes(item.menit_padam),
      up3_id: item.up3_id  // Tambahkan penyulang_id di sini
    });
    cat.push(item.nama_up3); // pastikan format nama_up3 sesuai
  });
  return { res, cat };
};
...
export default function ChartNyobaPadamPenyulang({
      ...
      point: {
        events: {
          click: function (event: any) {
            const up3_id1 = event.point.options.up3_id;
  ...
  return (
    <div className='px-4'>
      ...
      <h2 className='mb-4 text-center'>Lama Padam Per Area (menit)</h2>
